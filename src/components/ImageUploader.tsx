...
export const ImageUploader: React.FC<ImageUploaderProps> = ({ 
  ...
  return (
    <div className="w-full relative">
      ...
      {preview ? (
        <div className={`relative w-full ${aspectRatioClass} rounded-lg overflow-hidden border border-[var(--nb-border)]`}>
        ...
        <label
          className={`flex flex-col items-center justify-center w-full ${aspectRatioClass} border-2 border-[var(--nb-border)] border-dashed rounded-xl cursor-pointer bg-[var(--nb-surface)] hover:bg-[var(--nb-surface-alt)] transition-colors`}
          onDragOver={handleDragOver}
          onDrop={handleDrop}
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center">
            <span className="material-symbols-outlined text-[var(--nb-text-secondary)] mb-2" style={{fontSize: '36px'}}>cloud_upload</span>
            <p className="mb-2 text-sm text-[var(--nb-text)]">
              <span className="font-semibold">Click to upload</span> or drag and drop
            </p>
            <p className="text-xs text-[var(--nb-text-secondary)]">PNG, JPG, GIF up to 10MB</p>
          </div>
