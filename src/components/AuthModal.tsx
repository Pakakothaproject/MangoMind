    ...
    return (
        <div role="dialog" aria-modal="true" aria-labelledby="auth-modal-title">
            ...
            <div className="image-modal-content">
                <div className="neo-card p-6 w-full max-w-md space-y-4 m-4 relative">
                    <button onClick={onClose} className="absolute top-3 right-3 neo-button neo-icon-button neo-button-secondary"><XIcon /></button>
                    ...
                    <div className="relative">
                        ...
                        <div className="relative flex justify-center text-sm">
                            <span className="px-2 bg-[var(--nb-surface)] text-[var(--nb-text-secondary)]">Or continue with</span>
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="w-full neo-button neo-button-secondary text-lg flex items-center justify-center gap-3"
                    >
                    ...
                    {error && <p className="text-sm text-center text-[var(--nb-secondary)]">{error}</p>}
                    {message && <p className="text-sm text-center text-[var(--nb-primary)]">{message}</p>}
                    <div className="text-center text-sm">
                        {mode === 'signin' ? (
                            <p>Don't have an account? <button onClick={() => setMode('signup')} className="font-semibold text-[var(--nb-primary)]">Sign Up</button></p>
                        ) : (
                            <p>Already have an account? <button onClick={() => setMode('signin')} className="font-semibold text-[var(--nb-primary)]">Sign In</button></p>
                        )}
                    </div>
                </div>
            </div>
