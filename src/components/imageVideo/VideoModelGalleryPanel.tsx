...
const VideoModelGalleryPanel: React.FC = () => {
    ...
    const isDirectSettingsAccess = window.location.pathname === '/settings/video-models';
        ...
        return (
            <div className="h-full w-full bg-[var(--nb-bg)] text-[var(--nb-text)] overflow-y-auto">
                <header className="flex-shrink-0 p-4 flex items-center justify-between border-b border-[var(--nb-border)]">
                    <h2 className="text-xl font-bold text-[var(--nb-text)]">Video Generation Models</h2>
                    <button onClick={handleClose} className="neo-button neo-icon-button neo-button-secondary">
                        <XIcon />
                    </button>
                </header>
                <main className="flex-grow p-4">
                    <ImageVideoModelPreferences modelType="video" />
                </main>
            </div>
        );
    }
...
export default VideoModelGalleryPanel;