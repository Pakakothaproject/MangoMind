...
export const ModelGalleryPanel: React.FC = () => {
    const isModelGalleryOpen = useIsModelGalleryOpen();
        ...
        >
            ...
            >
                <header className="flex-shrink-0 p-4 flex items-center justify-between border-b border-[var(--nb-border)]">
                    ...
                    <button onClick={closeModelGallery} className="neo-button neo-icon-button neo-button-secondary">
                        <XIcon />
                    </button>
                </header>
                <main className="flex-grow p-4 overflow-y-auto">
                    <ImageVideoModelPreferences modelType="all" />
                </main>
            </div>
        </div>
    );
