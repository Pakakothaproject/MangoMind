...
serve(async (req) => {
  ...
  try {
    ...
    try {
        ...
        if (stream && responseText.trim().startsWith('data:')) {
            console.log('Detected SSE format without proper Content-Type header, forwarding as stream');
            return new Response(responseText, {
                status: response.status,
                headers: {
                    ...corsHeaders,
        ...
        // If the response was OK but not JSON and not SSE, it's an unexpected situation.
