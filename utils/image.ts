...
export const resizeImage = async (image: UploadedImage, maxDimension: number): Promise<UploadedImage> => {
    ...
    if (!match) throw new Error('Failed to create resized data URL.');
...
export const dataUrlToUploadedImage = (dataUrl: string): UploadedImage | null => {
  const match = dataUrl.match(/^data:(.+);base64,(.+)$/);
  if (match) {
      const [, type, base64] = match;
      return { base64, type };
