...
const SidebarButton: React.FC<{
    ...
    >
        ...
        {isActive && !isDragging && <div className="absolute -left-2.5 top-1/4 bottom-1/4 w-1 bg-[#F8C644] rounded-r-full transition-all duration-300" />}
        <button 
            onClick={onClick}
            className={`flex flex-col items-center justify-center w-full p-2 rounded-lg transition-colors duration-200 ${isActive ? 'bg-[#3f3f3f] text-white' : 'text-[var(--nb-text-secondary)] hover:bg-[#3a3a3a] hover:text-white'}`}
            title={config.label}
        >
            <span className="material-symbols-outlined !text-lg">{config.icon}</span>
            <span className="text-[9px] font-bold uppercase tracking-wider leading-tight">{config.label}</span>
        </button>
    </div>
);
...
const PlaygroundSidebar: React.FC = () => {
    ...
    return (
        <div className="h-full flex-shrink-0 p-3 pl-3 pb-6">
            <aside 
                className="w-16 h-full bg-[#2C2C2C] p-1.5 pb-4 flex flex-col items-center gap-0.5 rounded-t-xl border-t border-x border-white/30 shadow-2xl"
            >
