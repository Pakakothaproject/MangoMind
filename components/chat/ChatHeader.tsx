...
export const ChatHeader: React.FC<ChatHeaderProps> = React.memo(({ title, models, onBack, onModelSelectClick, onSystemPromptClick, onDeleteChat }) => {
    ...
    return (
        <header className="flex-shrink-0 flex items-center justify-between py-2 px-3 pr-16 md:py-3 md:px-4 md:pr-4 border-b border-[var(--jackfruit-darker)]">
            <div className="flex items-center gap-1 md:gap-4 min-w-0 flex-1">
                <button onClick={onBack} className="md:hidden p-0.5 text-[var(--jackfruit-muted)] hover:text-white flex-shrink-0">
                    <ArrowLeftIcon />
                </button>
                <div className="flex flex-col md:flex-row md:items-center gap-0.5 md:gap-3 min-w-0 flex-1">
                    <h2 className="font-bold text-xs md:text-lg text-white truncate leading-tight">{title}</h2>
                    <div onClick={onModelSelectClick} className="chat-header-models-container flex-shrink-0 flex-row">
                        {isAutoMode ? (
                            ...
                            modelDetails.map((modelInfo, index) => {
                                ...
                                return (
                                    <div key={modelInfo.id} className={`chat-header-model-pill ${colorClass}`}>
                                        {modelInfo.logo_url && <img src={modelInfo.logo_url} alt={`${modelInfo.category} logo`} className="w-2.5 h-2.5 md:w-3 md:h-3 object-contain rounded-sm" />}
                                        <span className="truncate max-w-[50px] md:max-w-none">{modelInfo.name || modelInfo.id}</span>
                                    </div>
                                );
                            })
                        )}
                        <ChevronDownIcon className="w-2.5 h-2.5 md:w-4 md:h-4 text-[var(--jackfruit-muted)]" />
                    </div>
                </div>
            </div>
